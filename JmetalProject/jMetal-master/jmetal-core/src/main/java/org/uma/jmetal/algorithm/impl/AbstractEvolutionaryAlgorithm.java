...
public abstract class AbstractEvolutionaryAlgorithm<S extends Solution<?>, R>  implements Algorithm<R>{
  ...
  @Override public void run() {
    ...
    //List<S> gauardadaOriginalParaMarcarMemoria;
    ...
    while (!isStoppingConditionReached()) {
     // gauardadaOriginalParaMarcarMemoria=population;//mio
      matingPopulation = selection(population);
      offspringPopulation = reproduction(matingPopulation);
      offspringPopulation = evaluatePopulation(offspringPopulation);
      population = replacement(population, offspringPopulation);
      //// TODO: 23/04/2017 sacar llamada al GC
      System.gc();
     // MemoriaCDN memoria = new MemoriaCDN();
     // memoria.determinarMemoriaLibre(population,gauardadaOriginalParaMarcarMemoria,offspringPopulation);
      updateProgress();
    }
  }
}
